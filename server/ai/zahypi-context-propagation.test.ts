...
describe("ZahyPi tenant propagation", () => {
  ...
  it("keeps OpenAI health probes non-generative", () => {
    for (const file of ["../cron/ai-health-monitor.ts", "../routers-ai-settings.ts"]) {
      const source = readFileSync(new URL(file, import.meta.url), "utf8");
      expect(source).not.toContain("api.openai.com/v1/chat/completions");
      expect(source).not.toContain("messages: [{ role: 'user', content: 'ping' }]");
    }
    const openAi = readFileSync(new URL("./openai.ts", import.meta.url), "utf8");
    expect(openAi).toContain("`${OPENAI_API_URL}/models`");
    expect(openAi).toContain("redirect: 'error'");
    const settings = readFileSync(new URL("../routers-ai-settings.ts", import.meta.url), "utf8");
    expect(settings).toContain(".max(512)");
    expect(settings).toContain("/^sk-[A-Za-z0-9_-]+$/");
  });
