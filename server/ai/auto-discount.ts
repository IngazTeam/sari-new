...
export async function generateAutoDiscount(params: {
  ...
  try {
    ...
    if ((settings as any)?.autoDiscountEnabled !== 1 && (settings as any)?.autoDiscountEnabled !== true) {
    ...
    const rateLimitKey = `${params.merchantId}:${customerPhone}`;
    ...
    const hasUnusedAutoCode = existingCodes.some((c: any) => c.isAutoGenerated && c.customerPhone
      && selectSalesDiscounts([c], { merchantId: params.merchantId, customerPhone }).length > 0);
    ...
    const strength = detectObjectionStrength(params.customerMessage) || 'mild';
