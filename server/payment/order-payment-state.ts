 ...
 */
export type TapOrderPaymentStateInput = {
  paymentId: number;
  tapChargeId: string;
...
export async function applyTapOrderPaymentState(input: TapOrderPaymentStateInput): Promise<AppliedTapOrderPaymentState> {
  ...
  try {
    ...
    const result = await applyTapOrderPaymentStateInTransaction(connection, input);
    try {
...
/** Caller owns begin/commit/rollback; financial effects and verified memory remain atomic. */
export async function applyTapOrderPaymentStateInTransaction(connection: PoolConnection, input: TapOrderPaymentStateInput): Promise<AppliedTapOrderPaymentState> {
  if (
