...
sariPlatformRouter.post('/sync/faqs', async (req: PlatformRequest, res: Response) => {
  ...
  try {
    ...
    await logBrainActivity(
      merchantId,
      'faq_created',
      `Platform Sync: ${result.created} جديد، ${result.updated} محدّث، ${result.removed} محذوف (${mode})`,
      { created: result.created, updated: result.updated, removed: result.removed, source: 'platform' },
    );
...
sariPlatformRouter.post('/sync/knowledge', async (req: PlatformRequest, res: Response) => {
  ...
  try {
    ...
    // 4. FAQs — only active rows explicitly enabled for Sari knowledge.
    // Fail visibly when the bounded contract is exceeded; partial ingestion would
    // leave the merchant believing all enabled answers were available to the bot.
    const { getByaanFaqsForKnowledge, BYAAN_KNOWLEDGE_FAQ_LIMIT } = await import('../integrations/byaan');
    const knowledgeFaqs = await getByaanFaqsForKnowledge(merchantId);
    if (knowledgeFaqs.exceedsLimit) {
      return res.status(422).json({
        success: false,
        error: 'Too many Byaan FAQs enabled for knowledge synchronization',
        errorAr: `عدد أسئلة بيان المفعلة لمعرفة ساري يتجاوز الحد (${BYAAN_KNOWLEDGE_FAQ_LIMIT}). عطّل الزائد ثم أعد المزامنة.`,
        limit: BYAAN_KNOWLEDGE_FAQ_LIMIT,
      });
    }
    ...
    // classifyContent truncates after 100k characters. Reject an oversized
    // canonical snapshot instead of claiming that a silently truncated prefix
    // represents all courses, pages, and enabled FAQs.
    const knowledgeInputLimit = 100_000;
    if (fullText.length > knowledgeInputLimit) {
      return res.status(422).json({
        success: false,
        error: 'Byaan knowledge snapshot exceeds the supported input size',
        errorAr: 'حجم معرفة بيان يتجاوز الحد المدعوم. قلّل أو اختصر المحتوى المفعّل ثم أعد المزامنة.',
        limit: knowledgeInputLimit,
      });
    }
    ...
    // Persist the exact current snapshot after AI analysis succeeds. This is the
    // authoritative bot input: a transaction inserts the new chunks and only
    // then disables older auto-generated Byaan sections. Merchant-edited rows
    // are intentionally preserved.
    const snapshotChunkSize = 12_000;
    const snapshotChunks = Array.from(
      { length: Math.ceil(fullText.length / snapshotChunkSize) },
      (_, index) => fullText.slice(index * snapshotChunkSize, (index + 1) * snapshotChunkSize),
    );
    const knowledgeDb = await import('../db/knowledge');
    const snapshotResult = await knowledgeDb.replaceByaanKnowledgeSnapshot(
      merchantId,
      snapshotChunks,
      crypto.randomUUID(),
    );
    ...
    await logBrainActivity(merchantId, 'knowledge_synced',
      ...
      { products: products.length, faqs: allFaqs.length, embedded: embeddedCount, source: 'byaan_sync', snapshot: snapshotResult, evolve, salesIntel: { usps: salesIntel?.usps?.length || 0, tips: salesIntel?.sellingTips?.length || 0, opportunities: salesIntel?.opportunities?.length || 0 } }
    );
    ...
    res.json({
      ...
      snapshotChunks: snapshotResult.inserted,
      staleSectionsDisabled: snapshotResult.disabled,
      expiredSnapshotChunksPruned: snapshotResult.pruned,
      salesIntel: {
