...
describe('real app router team boundaries', () => {
  ...
  it.each(['owner','manager','sales_supervisor'])('scopes booking payment evidence and review to the %s membership',async role=>{
    ...
    expect(mocks.bookingCheckoutRead).toHaveBeenCalledWith(20,10);expect(mocks.bookingCheckoutReview).toHaveBeenCalledWith(20,7,bookingReview);
  });
  it.each(['viewer',null])('denies booking payment evidence and review to %s access',async role=>{
    mocks.access.mockResolvedValue(role?{merchantId:20,role,memberId:3}:null);
    await expect(caller().bookings.getCheckoutAttempts({bookingId:10})).rejects.toMatchObject({code:'FORBIDDEN'});
    await expect(caller().bookings.reconcileCheckoutAttempt(bookingReview)).rejects.toMatchObject({code:'FORBIDDEN'});
    expect(mocks.bookingCheckoutRead).not.toHaveBeenCalled();expect(mocks.bookingCheckoutReview).not.toHaveBeenCalled();
  });
  it.each([{merchantId:99},{actorUserId:99},{orderId:99},{bookingId:1.2},{status:'CAPTURED'},{amount:1},{currency:'SAR'},{reviewed:false},{chargeId:'https://evil.test'},{evidence:''}])
    ('rejects injected booking payment review authority %j',async patch=>{
      mocks.access.mockResolvedValue({merchantId:20,role:'manager',memberId:3});await expect(caller().bookings.reconcileCheckoutAttempt({...bookingReview,...patch} as any)).rejects.toMatchObject({code:'BAD_REQUEST'});
      expect(mocks.bookingCheckoutReview).not.toHaveBeenCalled();
    });
  it('rejects injected booking evidence scope and redacts internal errors',async()=>{
    mocks.access.mockResolvedValue({merchantId:20,role:'owner',memberId:3});
    await expect(caller().bookings.getCheckoutAttempts({bookingId:10,merchantId:99} as any)).rejects.toMatchObject({code:'BAD_REQUEST'});
    mocks.bookingCheckoutRead.mockRejectedValueOnce(Error('private sql'));mocks.bookingCheckoutReview.mockRejectedValueOnce(Error('sk_test_private'));
    await expect(caller().bookings.getCheckoutAttempts({bookingId:10})).rejects.toMatchObject({code:'CONFLICT',message:'Booking checkout evidence unavailable'});
    await expect(caller().bookings.reconcileCheckoutAttempt(bookingReview)).rejects.toMatchObject({code:'CONFLICT',message:'Booking checkout reconciliation unavailable; refresh evidence before another review'});
  });
