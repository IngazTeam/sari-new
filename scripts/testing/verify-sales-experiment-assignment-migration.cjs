...
(async()=>{
 ...
 const database=`sari_sales_assignment_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/experiment-assignment';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0113_sales_experiment_assignments')throw Error('Unexpected migration');
  ...
  const noAutomaticAssignments=(await read('ai_sales_experiment_assignments')).length===0&&(await read('ai_sales_experiment_assignment_conversations')).length===0;
  const insert="INSERT INTO ai_sales_experiment_assignments (merchant_id,protocol_id,launch_id,customer_key,arm,conversation_reference,message_reference,observation_ends_at,assignment_digest,snapshot) VALUES (?,?,?,?,?,?,?,'2027-01-01',?,'{}')";
  const args=[m.insertId,protocol.insertId,launch.insertId,'a'.repeat(64),'baseline',123,456,'b'.repeat(64)];
  const [assignment]=await c.execute(insert,args),rejected={};
  for(const [label,changes,expected] of [['duplicateCustomer',{},'ER_DUP_ENTRY'],['duplicateSource',{3:'c'.repeat(64)},'ER_DUP_ENTRY'],['foreignProtocol',{1:99999999,3:'c'.repeat(64),6:457},'ER_NO_REFERENCED_ROW_2'],['foreignLaunch',{2:99999999,3:'c'.repeat(64),6:457},'ER_NO_REFERENCED_ROW_2'],['badArm',{3:'c'.repeat(64),4:'winner',6:457},'ER_CHECK_CONSTRAINT_VIOLATED']]){
  ...
  const bind="INSERT INTO ai_sales_experiment_assignment_conversations (merchant_id,protocol_id,conversation_reference,customer_key,assignment_id) VALUES (?,?,?,?,?)";
  const bargs=[m.insertId,protocol.insertId,123,'a'.repeat(64),assignment.insertId];await c.execute(bind,bargs);
  for(const [label,changes,expected] of [['duplicateConversation',{},'ER_DUP_ENTRY'],['foreignAssignment',{2:124,4:99999999},'ER_NO_REFERENCED_ROW_2']]){
    const next=[...bargs];for(const [i,v]of Object.entries(changes))next[Number(i)]=v;
    try{await c.execute(bind,next);rejected[label]=false;}catch(e){rejected[label]=e.code===expected;}
  }
  const assignmentBefore=JSON.stringify(await read('ai_sales_experiment_assignments')),bindingBefore=JSON.stringify(await read('ai_sales_experiment_assignment_conversations'));run(root,database);
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0112 in a fresh isolated database, seeds historical evidence, applies 0113 and reruns.',migrations:Number(count.count),
    preservedTables:preserved.filter(table=>before[table]===after[table]),noAutomaticAssignments,rejected,
    rerunUnchanged:JSON.stringify(after)===JSON.stringify(await snapshot())&&assignmentBefore===JSON.stringify(await read('ai_sales_experiment_assignments'))&&bindingBefore===JSON.stringify(await read('ai_sales_experiment_assignment_conversations')),
    globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0113_sales_experiment_assignments.sql'))).digest('hex')};
  report.passed=report.migrations===114&&report.preservedTables.length===preserved.length&&report.noAutomaticAssignments&&Object.values(rejected).every(Boolean)&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/experiment-assignment-migration.log',logs.join('\n'));console.log(JSON.stringify(report));
  if(!report.passed)throw Error('Migration verification failed');
  run(root,'sari_brain_test');console.log('Disposable acceptance database upgraded to 0113');
 }finally{await c.end();}
