...
async function render() {
  ...
  if(mode.startsWith('protocol-') || (mode.startsWith('cohort-') || mode.startsWith('inspection-') || mode.startsWith('plan-review-'))){createRoot(document.getElementById('root')!).render(<main className="mx-auto max-w-6xl p-3">{(mode==='protocol-standalone'||(mode.startsWith('cohort-') || mode.startsWith('inspection-') || mode.startsWith('plan-review-')))?<SalesExperimentProtocol/>:<LearningPolicyEvaluationPanel proposalId={16}/>}</main>);return;}
