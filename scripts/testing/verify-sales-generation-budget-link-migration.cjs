...
(async()=>{
 ...
 const database=`sari_sales_budget_link_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/sales-budget-link';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0117_sales_generation_budget_identity')throw Error('Unexpected migration');
  ...
  const rows=await read('ai_sales_experiment_generations'),newColumns=['expected_reservation_key'];
  ...
  const noAutomaticRecovery=rows.every(r=>newColumns.every(k=>r[k]===null));
  const [[idx]]=await c.query("SELECT COUNT(*) AS count FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='ai_sales_experiment_generations' AND index_name='uq_sales_generation_expected_reservation' AND non_unique=0");
  const indexPresent=Number(idx.count)===1;
  ...
  await c.execute('UPDATE ai_sales_experiment_generations SET expected_reservation_key=? WHERE id=?',['e'.repeat(64),g[0].id]);
  let duplicateRejected=false;
  try{await c.execute('UPDATE ai_sales_experiment_generations SET expected_reservation_key=? WHERE id=?',['e'.repeat(64),second.insertId]);}catch(e){duplicateRejected=e.code==='ER_DUP_ENTRY';}
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0116 in a fresh isolated database, seeds historical evidence, applies 0117 and reruns.',migrations:Number(count.count),
    preservedTables:preserved.filter(table=>before[table]===after[table]),generationPreserved,noAutomaticRecovery,indexPresent,duplicateRejected,
    ...
    globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0117_sales_generation_budget_identity.sql'))).digest('hex')};
  report.passed=report.migrations===118&&report.preservedTables.length===preserved.length&&report.generationPreserved&&report.noAutomaticRecovery&&report.indexPresent&&report.duplicateRejected&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/sales-budget-link-migration.log',logs.join('\n'));console.log(JSON.stringify(report));
  if(!report.passed)throw Error('Migration verification failed');
  run(root,'sari_brain_test');console.log('Disposable acceptance database upgraded to 0117');
 }finally{await c.end();}
