...
(async()=>{
 ...
 const database=`sari_sales_review_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/experiment-review';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0111_sales_experiment_reviews')throw Error('Unexpected migration');
  ...
  const preserved=['ai_learning_proposals','ai_learning_policy_reviews','ai_learning_policy_candidates','ai_learning_policy_evaluations','ai_learning_policy_output_reviews','ai_sales_experiment_protocols','ai_sales_experiment_cohorts','ai_budget_policies','ai_price_cards'];
  ...
  const noAutomaticReviews=(await read('ai_sales_experiment_reviews')).length===0;
  const args=[m.insertId,protocol.insertId,1,crypto.randomUUID(),'a'.repeat(64),'b'.repeat(64),'c'.repeat(64),'approved',u.insertId];
  const insert="INSERT INTO ai_sales_experiment_reviews (merchant_id,protocol_id,revision,request_id,payload_digest,basis_digest,review_digest,snapshot,verdict,actor_user_id) VALUES (?,?,?,?,?,?,?,'{}',?,?)";
  await c.execute(insert,args);
  const rejected={};
  for(const [label,changes,expected] of [['duplicateRequest',{2:2},'ER_DUP_ENTRY'],['duplicateRevision',{3:crypto.randomUUID()},'ER_DUP_ENTRY'],['foreignProtocol',{1:99999999,3:crypto.randomUUID()},'ER_NO_REFERENCED_ROW_2'],['badRevision',{2:0,3:crypto.randomUUID()},'ER_CHECK_CONSTRAINT_VIOLATED'],['unsafeRevision',{2:9007199254740992,3:crypto.randomUUID()},'ER_CHECK_CONSTRAINT_VIOLATED'],['badVerdict',{2:2,3:crypto.randomUUID(),7:'launched'},'ER_CHECK_CONSTRAINT_VIOLATED']]){
  ...
  const reviewBefore=JSON.stringify(await read('ai_sales_experiment_reviews'));run(root,database);
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0110 in a fresh isolated database, seeds historical evidence, applies 0111 and reruns.',migrations:Number(count.count),
   preservedTables:preserved.filter(table=>before[table]===after[table]),noAutomaticReviews,rejected,
   rerunUnchanged:JSON.stringify(after)===JSON.stringify(await snapshot())&&reviewBefore===JSON.stringify(await read('ai_sales_experiment_reviews')),
   globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0111_sales_experiment_reviews.sql'))).digest('hex')};
  report.passed=report.migrations===112&&report.preservedTables.length===preserved.length&&report.noAutomaticReviews&&Object.values(rejected).every(Boolean)&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/experiment-review-migration.log',logs.join('\n'));console.log(JSON.stringify(report));
  if(!report.passed)throw Error('Migration verification failed');
  run(root,'sari_brain_test');console.log('Disposable acceptance database upgraded to 0111');
 }finally{await c.end();}
