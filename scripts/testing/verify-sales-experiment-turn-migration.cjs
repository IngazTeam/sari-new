...
(async()=>{
 ...
 const database=`sari_sales_turn_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/experiment-turn';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0114_sales_experiment_turns')throw Error('Unexpected migration');
  ...
  const noAutomaticTurns=(await read('ai_sales_experiment_turns')).length===0;
  const insert="INSERT INTO ai_sales_experiment_turns (merchant_id,protocol_id,assignment_id,conversation_reference,message_reference,request_id,payload_digest,turn_digest,snapshot) VALUES (?,?,?,?,?,?,?,?,'{}')";
  const args=[m.insertId,protocol.insertId,assignment.insertId,123,456,crypto.randomUUID(),'a'.repeat(64),'b'.repeat(64)];await c.execute(insert,args);const rejected={};
  for(const [label,changes,expected] of [['duplicateRequest',{},'ER_DUP_ENTRY'],['duplicateMessage',{5:crypto.randomUUID()},'ER_DUP_ENTRY'],['foreignProtocol',{1:99999999,4:457,5:crypto.randomUUID()},'ER_NO_REFERENCED_ROW_2'],['foreignAssignment',{2:99999999,4:457,5:crypto.randomUUID()},'ER_NO_REFERENCED_ROW_2']]){
  ...
  const turnsBefore=JSON.stringify(await read('ai_sales_experiment_turns'));run(root,database);
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0113 in a fresh isolated database, seeds historical evidence, applies 0114 and reruns.',migrations:Number(count.count),
    preservedTables:preserved.filter(table=>before[table]===after[table]),noAutomaticTurns,rejected,
    rerunUnchanged:JSON.stringify(after)===JSON.stringify(await snapshot())&&turnsBefore===JSON.stringify(await read('ai_sales_experiment_turns')),
    globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0114_sales_experiment_turns.sql'))).digest('hex')};
  report.passed=report.migrations===115&&report.preservedTables.length===preserved.length&&report.noAutomaticTurns&&Object.values(rejected).every(Boolean)&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/experiment-turn-migration.log',logs.join('\n'));console.log(JSON.stringify(report));
  if(!report.passed)throw Error('Migration verification failed');
  run(root,'sari_brain_test');console.log('Disposable acceptance database upgraded to 0114');
 }finally{await c.end();}
