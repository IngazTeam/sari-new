...
(async()=>{
 ...
 const database=`sari_sales_reply_review_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/sales-reply-review';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0118_sales_generation_output_reviews')throw Error('Unexpected migration');
  ...
  const noAutomaticReviews=(await read('ai_sales_generation_output_reviews')).length===0;
  const [generations]=await c.execute('SELECT id FROM ai_sales_experiment_generations');
  const insertReview="INSERT INTO ai_sales_generation_output_reviews (merchant_id,generation_id,actor_user_id,revision,request_id,payload_digest,basis_digest,review_digest,snapshot,outcome) VALUES (?,?,?,?,?,?,?,?,'{}',?)";
  const reviewArgs=[m.insertId,generations[0].id,u.insertId,1,crypto.randomUUID(),'a'.repeat(64),'b'.repeat(64),'c'.repeat(64),'approved'];
  await c.execute(insertReview,reviewArgs);
  const rejected=[];
  for(const kind of ['request','revision','zero','outcome','generation']){
    const values=[...reviewArgs];values[3]=2;values[4]=crypto.randomUUID();
    if(kind==='request')values[4]=reviewArgs[4];if(kind==='revision')values[3]=1;if(kind==='zero')values[3]=0;
    if(kind==='outcome')values[8]='invalid';if(kind==='generation')values[1]=0;
    try{await c.execute(insertReview,values);}catch(e){if(['ER_DUP_ENTRY','ER_CHECK_CONSTRAINT_VIOLATED','ER_NO_REFERENCED_ROW_2'].includes(e.code))rejected.push(kind);else throw e;}
  }
  const reviewsBefore=JSON.stringify(await read('ai_sales_generation_output_reviews'));run(root,database);
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0117 in a fresh isolated database, seeds historical generation evidence, applies 0118 and reruns.',migrations:Number(count.count),
    preservedTables:preserved.filter(table=>before[table]===after[table]),noAutomaticReviews,rejectedConstraints:rejected,
    rerunUnchanged:JSON.stringify(after)===JSON.stringify(await snapshot())&&reviewsBefore===JSON.stringify(await read('ai_sales_generation_output_reviews')),
    globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0118_sales_generation_output_reviews.sql'))).digest('hex')};
  report.passed=report.migrations===119&&report.preservedTables.length===preserved.length&&report.noAutomaticReviews&&rejected.length===5&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/sales-reply-review-migration.log',logs.join('\n'));console.log(JSON.stringify(report));
  if(!report.passed)throw Error('Migration verification failed');
  run(root,'sari_brain_test');console.log('Disposable acceptance database upgraded to 0118');
 }finally{await c.end();}
