...
(async()=>{
 ...
 const database=`sari_sales_generation_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/experiment-generation';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0115_sales_experiment_generations')throw Error('Unexpected migration');
  ...
  const noAutomaticGenerations=(await read('ai_sales_experiment_generations')).length===0;
  const insert="INSERT INTO ai_sales_experiment_generations (merchant_id,turn_id,actor_user_id,request_id,payload_digest,authorization_digest,snapshot,state,claim_token,reservation_key) VALUES (?,?,?,?,?,?,'{}','dispatching',?,?)";
  const args=[m.insertId,turn.insertId,u.insertId,crypto.randomUUID(),'a'.repeat(64),'b'.repeat(64),crypto.randomUUID(),crypto.randomUUID()];await c.execute(insert,args);const rejected={};
  for(const [label,changes,expected] of [['duplicateRequest',{1:otherTurn.insertId,7:crypto.randomUUID()},'ER_DUP_ENTRY'],['duplicateTurn',{3:crypto.randomUUID(),7:crypto.randomUUID()},'ER_DUP_ENTRY'],['duplicateReservation',{1:otherTurn.insertId,3:crypto.randomUUID()},'ER_DUP_ENTRY'],['foreignTurn',{1:99999999,3:crypto.randomUUID(),7:crypto.randomUUID()},'ER_NO_REFERENCED_ROW_2'],['foreignActor',{1:otherTurn.insertId,2:99999999,3:crypto.randomUUID(),7:crypto.randomUUID()},'ER_NO_REFERENCED_ROW_2']]){
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0114 in a fresh isolated database, seeds historical evidence, applies 0115 and reruns.',migrations:Number(count.count),
    preservedTables:preserved.filter(table=>before[table]===after[table]),noAutomaticGenerations,rejected,
    ...
    globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0115_sales_experiment_generations.sql'))).digest('hex')};
  report.passed=report.migrations===116&&report.preservedTables.length===preserved.length&&report.noAutomaticGenerations&&Object.values(rejected).every(Boolean)&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/experiment-generation-migration.log',logs.join('\n'));console.log(JSON.stringify(report));
  if(!report.passed)throw Error('Migration verification failed');
  run(root,'sari_brain_test');console.log('Disposable acceptance database upgraded to 0115');
 }finally{await c.end();}
