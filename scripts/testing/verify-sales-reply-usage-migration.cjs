...
(async()=>{
 ...
 const database=`sari_sales_reply_usage_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/reply-usage';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0122_sales_reply_usage')throw Error('Unexpected migration');
  ...
  preserved.push('ai_sales_reply_deliveries','merchant_subscriptions','subscription_plans');
  const [plan]=await c.execute("INSERT INTO subscription_plans (name,name_en,monthly_price,yearly_price,max_customers,message_limit) VALUES ('Historical quota','Historical quota',10,100,100,50)");
  const [subscription]=await c.execute("INSERT INTO merchant_subscriptions (merchant_id,plan_id,status,billing_cycle,start_date,end_date,messages_used) VALUES (?,?,'active','monthly','2026-09-01','2026-10-01',7)",[m.insertId,plan.insertId]);
  const before=await snapshot(),oldRows=await read('ai_sales_reply_deliveries');run(root,database);const after=await snapshot(),rows=await read('ai_sales_reply_deliveries');
  after.ai_sales_reply_deliveries=JSON.stringify(rows.map(row=>Object.fromEntries(Object.keys(oldRows[0]).map(key=>[key,row[key]]))));
  const noInventedCharge=rows.every(row=>row.usage_state==='legacy'&&row.usage_units===0&&row.usage_subscription_id===null&&row.usage_digest===null)
    &&rows.map(row=>row.state).join(',')==='authorized,dispatching';
  const rejected=[];
  const valid={usage_state:'held',usage_units:2,usage_subscription_id:subscription.insertId,usage_period_start:'2026-09-01',usage_reserved_at:'2026-09-26',usage_digest:'a'.repeat(64),usage_settled_at:null};
  for(const [name,changes] of [
    ['state',{usage_state:'other'}],['units',{usage_units:1}],['subscription',{usage_subscription_id:null}],
    ['period',{usage_period_start:null}],['digest',{usage_digest:null}],['held-with-settlement',{usage_settled_at:'2026-09-26'}],
    ['charged-without-settlement',{usage_state:'charged'}],['legacy-with-reservation',{usage_state:'legacy'}],
  ]) {
    const value={...valid,...changes};await c.beginTransaction();
    try{await c.execute('UPDATE ai_sales_reply_deliveries SET '+Object.keys(value).map(k=>k+'=?').join(',')+' WHERE id=?',[...Object.values(value),rows[0].id]);}
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000-0121, preserves historical delivery and subscription usage, applies 0122 and safely reruns. No historical billing or send permission is invented.',migrations:Number(count.count),
    preservedTables:preserved.filter(table=>before[table]===after[table]),historicalDeliveries:rows.length,noInventedCharge,rejectedConstraints:rejected,
    ...
    migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0122_sales_reply_usage.sql'))).digest('hex')};
  report.passed=report.migrations===123&&report.preservedTables.length===preserved.length&&noInventedCharge&&rejected.length===8&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/reply-usage-migration.log',logs.join('\n'));console.log(JSON.stringify(report));if(!report.passed)throw Error('Migration verification failed');
 }finally{await c.end();}
