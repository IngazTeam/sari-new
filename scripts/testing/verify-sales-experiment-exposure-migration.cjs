...
(async()=>{
 ...
 const database=`sari_sales_exposure_${Date.now()}_test`,c=await mysql.createConnection(config),dir=path.resolve(`.tmp/${database}`),out='docs/audits/sales-brain-implementation-2026-09-23/transport-exposure';
 ...
 try{
  ...
  if(journal.entries.at(-1).tag!=='0124_sales_experiment_exposures')throw Error('Unexpected migration');
  ...
  await c.execute("UPDATE ai_sales_reply_deliveries SET projection_state='projected',projection_completed_at=UTC_TIMESTAMP(3),projection_next_at=NULL,projection_attempts=3 WHERE state='dispatching'");
  const before=await snapshot(),oldRows=await read('ai_sales_reply_deliveries');run(root,database);
  const rows=await read('ai_sales_reply_deliveries'),after=await snapshot();
  const projected=rows.find(row=>row.state==='dispatching'),authorized=rows.find(row=>row.state==='authorized');
  const requeued=projected.projection_state==='pending'&&projected.projection_attempts===0&&projected.projection_completed_at===null&&projected.projection_next_at!==null;
  const historicalFields=Object.keys(oldRows[0]).filter(k=>!['projection_state','projection_attempts','projection_completed_at','projection_next_at','projection_last_error'].includes(k));
  const preservedDeliveryAuthority=rows.every(row=>historicalFields.every(k=>JSON.stringify(row[k])===JSON.stringify(oldRows.find(old=>old.id===row.id)[k])))
    &&JSON.stringify(authorized)===JSON.stringify(oldRows.find(row=>row.state==='authorized'));
  const initiallyEmpty=(await read('ai_sales_experiment_exposures')).length===0;
  const insertExposure="INSERT INTO ai_sales_experiment_exposures (merchant_id,delivery_id,protocol_id,assignment_id,outbox_id,exposure_digest,snapshot) VALUES (?,?,?,?,?,?,'{}')";
  const exposureArgs=[m.insertId,projected.id,protocol.insertId,assignment.insertId,123,'a'.repeat(64)];await c.execute(insertExposure,exposureArgs);
  const rejected=[];
  for(const [name,values] of [['delivery',exposureArgs],['outbox',[m.insertId,authorized.id,protocol.insertId,assignment.insertId,123,'b'.repeat(64)]]]) {
    try{await c.execute(insertExposure,values);}catch(e){if(e.code!=='ER_DUP_ENTRY')throw e;rejected.push(name);}
  }
  preserved.push('ai_sales_reply_deliveries','ai_sales_experiment_exposures');const finalBefore=await snapshot();run(root,database);
  ...
  const report={verifiedAt:new Date().toISOString(),database,scope:'Actual updater applies 0000–0123; seeds authorized/projected transport history; applies 0124 and safely reruns. Only projected SQL recovery is requeued; no exposure or send is invented.',
    migrations:Number(count.count),preservedTables:Object.keys(before).filter(table=>before[table]===after[table]),historicalDeliveries:rows.length,
    preservedDeliveryAuthority,requeued,initiallyEmpty,rejectedUniqueIndexes:rejected,rerunUnchanged:JSON.stringify(finalBefore)===JSON.stringify(await snapshot()),
    globalDailyMicroUsd:String(policy.daily_limit_micro_usd),migrationSha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'drizzle/0124_sales_experiment_exposures.sql'))).digest('hex')};
  report.passed=report.migrations===125&&report.preservedTables.length===Object.keys(before).length&&preservedDeliveryAuthority&&requeued&&initiallyEmpty&&rejected.length===2&&report.rerunUnchanged&&report.globalDailyMicroUsd==='100000000';
  fs.writeFileSync(out+'/migration.json',JSON.stringify(report,null,2)+'\n');fs.writeFileSync('.tmp/exposure-migration.log',logs.join('\n'));console.log(JSON.stringify(report));if(!report.passed)throw Error('Migration verification failed');
 }finally{await c.end();}
