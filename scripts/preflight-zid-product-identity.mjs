...
try {
    ...
    SELECT COUNT(*) AS duplicateGroups,
      ...
      FROM (
        SELECT COUNT(*) AS duplicateCount
          FROM products
         WHERE sallaProductId IS NOT NULL
         GROUP BY merchantId, sallaProductId
        HAVING COUNT(*) > 1
      ) duplicateIdentities
  `);
  ...
  if (duplicateGroups > 0) {
    console.error('ZID_PRODUCT_IDENTITY_PREFLIGHT_FAILED');
    process.exitCode = 1;
  }
} catch {
  console.error('ZID_PRODUCT_IDENTITY_PREFLIGHT_UNAVAILABLE');
  process.exitCode = 2;
} finally {
  await pool.end();
}
