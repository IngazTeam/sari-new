...
export async function wireTransactions(
  ...
  if (!box) return;
  const localizedText = (ar: string, en: string) => (lang === "ar" ? ar : en),
  ...
  if (path.startsWith("/subscribe")) {
    try {
      ...
      let selected = requested
        ...
        : plans[0];
      if (!selected) {
        box.innerHTML = `<p>${localizedText("الباقة غير متاحة. اختر من الباقات الحالية.", "This plan is unavailable. Choose a current plan.")}</p><a class="button green" href="${href("/pricing")}">${localizedText("الباقات", "See plans")}</a>`;
        return;
      }
      ...
      const render = () => {
        ...
        box.innerHTML = `<h2>${localizedText("راجع اختيارك", "Review your choice")}</h2><label class="page-field"><span>${localizedText("الباقة", "Plan")}</span><select id="subscribe-plan">${plans.map(x => `<option value="${x.id}" ${x.id === p.id ? "selected" : ""}>${e(lang === "ar" ? x.name : x.nameEn || "Plan " + x.id)}</option>`).join("")}</select></label><label class="page-field"><span>${localizedText("دورة الفوترة", "Billing period")}</span><select id="subscribe-period"><option value="monthly" ${billing === "monthly" ? "selected" : ""}>${localizedText("شهري", "Monthly")}</option><option value="yearly" ${billing === "yearly" ? "selected" : ""}>${localizedText("سنوي", "Yearly")}</option></select></label><div class="checkout-total"><strong>${price(amount, p.currency)}</strong><span>/ ${billing === "monthly" ? localizedText("شهر", "month") : localizedText("سنة", "year")}</span></div><p>${e(lang === "ar" ? p.description : p.descriptionEn || "")}</p>${renderPlanLimits(p, lang)}<p class="small-note">${localizedText("راجع تفاصيل بوابة الدفع قبل التأكيد. لا يتم الخصم لمجرد عرض هذه الصفحة.", "Review the provider’s checkout details before confirmation. Viewing this page does not charge you.")}</p>${user ? `<button class="button green full" id="start-checkout">${localizedText("المتابعة إلى بوابة الدفع", "Continue to payment")}${icon("arrow-up-left", "directional")}</button>` : `<div class="inline-notice">${localizedText("سجّل الدخول أو أنشئ حسابًا لإكمال الاشتراك.", "Sign in or create an account to continue your subscription.")}</div><a class="button green full" href="${e(href("/login?next=" + encodeURIComponent(target)))}">${localizedText("تسجيل الدخول", "Sign in")}</a><a class="text-link" href="${e(href("/signup?next=" + encodeURIComponent(target)))}">${localizedText("إنشاء حساب", "Create an account")}</a>`}`;
        root
      ...
      box.innerHTML = `<p>${localizedText("تعذر تحميل تفاصيل الاشتراك الحالية.", "Current subscription details could not be loaded.")}</p><a class="button outline" href="${href("/pricing")}">${localizedText("العودة للباقات", "Back to plans")}</a>`;
    }
    return;
  }
  if (/^\/pay\/[^/]+$/.test(path) || path === "/pay") {
    ...
    try {
      ...
      box.innerHTML = `<h2>${e(link.title)}</h2><p>${e(link.description || "")}</p><div class="checkout-total"><strong>${price(link.amount / 100, link.currency)}</strong></div><form id="order-checkout">${input("customerName", localizedText("الاسم", "Name"))}${input("customerPhone", localizedText("رقم الجوال السعودي", "Saudi mobile number"), "tel")}${input("customerEmail", localizedText("البريد الإلكتروني (اختياري)", "Email address (optional)"), "email", true)}<button class="button green full" type="submit">${localizedText("المتابعة إلى الدفع", "Continue to payment")}</button></form><p class="small-note">${localizedText("تُدخل بيانات البطاقة لدى بوابة الدفع. ساري لا يحفظ رقم بطاقتك كاملًا.", "Enter card details with the payment provider. Sary does not store your full card number.")}</p>`;
      root
  ...
  if (
    ...
    box.innerHTML = `<span class="auth-state-icon">${icon("x")}</span><h2>${localizedText("رابط التحقق غير صالح.", "The verification link is invalid.")}</h2><p>${localizedText("تحقق من رابط العودة أو تواصل مع الدعم. لا يمكن استنتاج نجاح الدفع من عنوان الصفحة.", "Check the return link or contact support. The page address alone cannot confirm payment.")}</p>`;
    return;
  }
  let polls = 0,
    finished = false,
