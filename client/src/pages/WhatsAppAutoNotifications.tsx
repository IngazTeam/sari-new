...
export default function WhatsAppAutoNotifications() {
  ...
  const createMutation = trpc.advancedNotifications.createWhatsappAutoNotification.useMutation({ onSuccess: () => { toast.success("تم إنشاء الإشعار بنجاح"); setIsDialogOpen(false); resetForm(); refetch(); } });
  const updateMutation = trpc.advancedNotifications.updateWhatsappAutoNotification.useMutation({ onSuccess: () => { toast.success("تم تحديث الإشعار بنجاح"); setIsDialogOpen(false); resetForm(); refetch(); } });
  const deleteMutation = trpc.advancedNotifications.deleteWhatsappAutoNotification.useMutation({ onSuccess: () => { toast.success("تم حذف الإشعار بنجاح"); refetch(); } });
  ...
  const handleSubmit = () => { if (!formData.messageTemplate) { toast.error("يرجى إدخال نص الرسالة"); return; } if (editingNotification) { updateMutation.mutate({ id: editingNotification.id, ...formData }); } else { createMutation.mutate(formData); } };
  ...
  return (
    <div className="container mx-auto py-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">إشعارات WhatsApp التلقائية</h1><p className="text-muted-foreground">إرسال رسائل تلقائية للعملاء عند تغيير حالة الطلبات أو المواعيد</p></div>
        <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild><Button><Plus className="h-4 w-4 ml-2" />إشعار جديد</Button></DialogTrigger>
          <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{editingNotification ? "تعديل الإشعار" : "إنشاء إشعار جديد"}</DialogTitle><DialogDescription>قم بتخصيص الرسالة التلقائية للعملاء</DialogDescription></DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2"><Label>نوع الحدث</Label><Select value={formData.triggerType} onValueChange={(v) => setFormData(p => ({...p, triggerType: v, messageTemplate: defaultTemplates?.[v as keyof typeof defaultTemplates] || ""}))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(triggerLabels).map(([key, { label, category }]) => <SelectItem key={key} value={key}>{category} - {label}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-2"><div className="flex items-center justify-between"><Label>نص الرسالة</Label><Button variant="ghost" size="sm" onClick={loadDefaultTemplate}><RotateCcw className="h-4 w-4 ml-1" />استعادة القالب الافتراضي</Button></div><Textarea value={formData.messageTemplate} onChange={(e) => setFormData(p => ({...p, messageTemplate: e.target.value}))} placeholder="أدخل نص الرسالة..." className="min-h-[200px]" dir="rtl" /><p className="text-xs text-muted-foreground">المتغيرات المتاحة: {"{{customerName}}, {{orderNumber}}, {{total}}, {{currency}}, {{trackingNumber}}, {{deliveryDate}}, {{serviceName}}, {{appointmentDate}}, {{appointmentTime}}, {{location}}"}</p></div>
              <div className="flex items-center justify-between"><Label>تفعيل الإشعار</Label><Switch checked={formData.isActive} onCheckedChange={(v) => setFormData(p => ({...p, isActive: v}))} /></div>
            </div>
            <DialogFooter><Button variant="outline" onClick={() => setIsDialogOpen(false)}>إلغاء</Button><Button onClick={handleSubmit} disabled={createMutation.isPending || updateMutation.isPending}>{editingNotification ? "تحديث" : "إنشاء"}</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card><CardHeader><div className="flex items-center gap-2"><ShoppingBag className="h-5 w-5 text-primary" /><CardTitle>إشعارات الطلبات</CardTitle></div><CardDescription>رسائل تلقائية عند تغيير حالة الطلبات</CardDescription></CardHeader><CardContent className="space-y-3">
          {orderNotifications.length === 0 ? <p className="text-center text-muted-foreground py-4">لا توجد إشعارات للطلبات</p> : orderNotifications.map((notification: any) => (
        ...
        <Card><CardHeader><div className="flex items-center gap-2"><Calendar className="h-5 w-5 text-primary" /><CardTitle>إشعارات المواعيد</CardTitle></div><CardDescription>رسائل تلقائية عند حجز أو تغيير المواعيد</CardDescription></CardHeader><CardContent className="space-y-3">
          {appointmentNotifications.length === 0 ? <p className="text-center text-muted-foreground py-4">لا توجد إشعارات للمواعيد</p> : appointmentNotifications.map((notification: any) => (
            <div key={notification.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              <div className="flex items-center gap-3">{triggerLabels[notification.trigger_type]?.icon}<div><p className="font-medium">{triggerLabels[notification.trigger_type]?.label || notification.trigger_type}</p><p className="text-xs text-muted-foreground line-clamp-1">{notification.message_template.substring(0, 50)}...</p></div></div>
              <div className="flex items-center gap-2"><Badge variant={notification.is_active ? "default" : "secondary"}>{notification.is_active ? "نشط" : "متوقف"}</Badge><Button variant="ghost" size="sm" onClick={() => handleEdit(notification)}><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="sm" className="text-red-600" onClick={() => deleteMutation.mutate({ id: notification.id })}><Trash2 className="h-4 w-4" /></Button></div>
            </div>
