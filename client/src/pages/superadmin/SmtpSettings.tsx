...
export default function SmtpSettings() {
  ...
  const updateSettings = trpc.smtp.updateSettings.useMutation({
    onSuccess: () => {
      toast.success(" „ Õ›Ÿ ≈⁄œ«œ«  SMTP »‰Ã«Õ");
      refetch();
    },
    onError: (error: any) => {
      toast.error(`›‘· Õ›Ÿ «·≈⁄œ«œ« : ${error.message}`);
    },
  });

  const testConnection = trpc.smtp.testConnection.useMutation({
    onSuccess: () => {
      toast.success(" „ ≈—”«· «·»—Ìœ «· Ã—Ì»Ì »‰Ã«Õ!  Õﬁﬁ „‰ ’‰œÊﬁ «·Ê«—œ.");
    },
    onError: (error: any) => {
      toast.error(`›‘· ≈—”«· «·»—Ìœ: ${error.message}`);
    },
  });
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {stats.totalEmails > 0 ? Math.round((stats.sentEmails / stats.totalEmails) * 100) : 0}% „⁄œ· «·‰Ã«Õ
                  </p>
                </CardContent>
              </Card>
              <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="rounded-lg border p-4 bg-muted/50">
                ...
                <ul className="text-sm space-y-1 text-muted-foreground">
                  ...
                  <li>ï «· «—ÌŒ Ê«·Êﬁ : {new Date().toLocaleString("ar-SA")}</li>
                </ul>
              </div>
            </CardContent>
          </Card>
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {log.status === "sent" ? " „ «·≈—”«·" : log.status === "failed" ? "›‘·" : "„⁄·ﬁ"}
                          </Badge>
                        </div>
                        <div className="text-sm text-muted-foreground mt-1">
                          ≈·Ï: {log.toEmail} ï {new Date(log.createdAt).toLocaleString("ar-SA")}
                        </div>
                        {log.error && (
                          <div className="text-sm text-red-600 mt-1">Œÿ√: {log.error}</div>
                        )}
                      </div>
                    </div>
                  ))
