...
export function CheckoutDiscountBreakdown({discount,totalMinor,approved}:{discount:CheckoutDiscountDisplay;totalMinor:number;approved:boolean}) {
  ...
  return <section data-checkout-discount className="space-y-3 rounded-xl border p-4 text-sm">
    ...
    <p data-discount-state={approved?'applied':'pending'} className="leading-relaxed text-muted-foreground">{approved?t('merchantUx.checkoutDiscount.applied'):t('merchantUx.checkoutDiscount.pending')}</p>
  </section>;
}
