...
export function renderCentralMarkup(
  ...
  const img = (name: string, alt: string, priority = false) =>
    `<picture><source type="image/webp" srcset="/central/${name}-640.webp 640w, /central/${name}-1024.webp 1024w" sizes="(max-width:700px) 90vw, 48vw"><img src="/central/${name}-1024.webp" alt="${e(alt)}" width="1024" height="${name === "founder" ? 1280 : 683}" ${priority ? 'fetchpriority="high"' : 'loading="lazy"'} decoding="async"></picture>`;
  const founderAlt = t(
    "رائد أعمال سعودي يحمل حاسوبًا",
    "A Saudi entrepreneur holding a laptop"
  );
  const teamAlt = t(
  ...
  const home = () =>
    `<section class="hero container"><div class="hero-copy"><div class="eyebrow"><span class="live-dot"></span>${t("شريك نموّك على واتساب", "Your growth partner on WhatsApp")}</div><h1>${t('شغلك يكبر.<br><strong>وساري <span class="underlined">معك.</span></strong>', 'Your business grows.<br><strong>Sary grows <span class="underlined">with you.</span></strong>')}</h1><p class="hero-description">${t("خلّك قريب من طموحك، ومن عملائك.<br>ساري يفهم، يرد، ويتابع البيع على واتساب.<br>وأنت تتفرّغ للي يخلّي شغلك يكبر.", "Stay close to your ambition and your customers.<br>Sary understands, replies and follows up on WhatsApp.<br>So you can focus on what makes your business grow.")}</p><div class="hero-actions">${button("/try-sari", t("جرّب ساري بنفسك", "Try Sary yourself"))}${button("/product/ai-agent", t("اكتشف كيف يساعدك", "See how it helps"), "outline")}</div><div class="hero-notes">${[t("يفهم لهجتك", "Understands your language"), t("يعرف معلومات نشاطك", "Uses your business knowledge"), t("فريقك دائمًا بالصورة", "Keeps your team involved")].map(n => `<span>${icon("check")}${n}</span>`).join("")}</div></div><div class="hero-visual"><div class="portrait-frame">${img("founder", founderAlt, true)}<div class="portrait-caption"><span class="tiny-label">MORE HUMAN. MORE POSSIBLE.</span><span>${t("الطموح منك.<br><b>والباقي نكمّله سوا.</b>", "Your ambition.<br><b>Our next step together.</b>")}</span></div>${icon("sparkles", "orbit-spark")}</div><div class="float-badge"><span class="badge-icon">${icon("sparkles")}</span><div><strong>${t("ساري معك", "Sary is with you")}</strong><span>${t("في تفاصيل يومك", "In the everyday details")}</span></div><i class="live-dot"></i></div><div class="order-float"><div class="order-top">${icon("bag")}<span>${t("تجربة عميل أقرب", "A closer customer experience")}</span></div><b>${t("من السؤال، إلى خطوة واضحة.", "From a question to a clear next step.")}</b><p>${t("معلوماتك تجمع المحادثة.", "Your knowledge connects the conversation.")}</p><div class="order-bottom"><span>${t("ساري + فريقك", "Sary + your team")}</span>${icon("check")}</div></div><span class="visual-footnote">${t("شخصية توضيحية مولّدة", "AI-generated illustrative portrait")}</span></div></section><section class="trust-strip container"><span>${t("قريب من الأدوات اللي تعرفها", "Works with the tools you know")}</span><div>${["WhatsApp", "Salla", "Zid", "Byaan", "Tap"].map(n => `<span>${n}</span>`).join("")}</div></section>${features(catalog["/product/ai-agent"])}${demo()}<section class="people-section section container" id="people"><div class="team-photo">${img("team", teamAlt)}<div class="photo-label">${t("البشر في قلب التجربة.", "People at the heart of it.")}</div></div><div class="people-copy">${tag(t("شغلك له قصته", "Your business has a story"))}<h2>${t('تختلف الأنشطة.<br><span class="green-text">والطموح يجمعنا.</span>', 'Different businesses.<br><span class="green-text">A shared ambition.</span>')}</h2><p>${t("من متجر إلى مركز تدريب أو مقدم خدمات. معلوماتك وطريقة شغلك هي البداية في كل محادثة.", "From a store to a training centre or a service business, your knowledge and way of working shape the conversation.")}</p><div class="people-pills">${["/solutions/restaurants", "/solutions/training-centers", "/solutions/consultants"].map(p => link(p, catalog[p].label)).join("")}</div>${button("/sectors", t("اكتشف قطاعك", "Explore your industry"), "outline")}</div></section><section class="container section center-heading">${tag(t("على قد شغلك. وعلى قد طموحك.", "Room for your business. Room for your ambition."))}<h2>${t("ابدأ بخطوة.<br>وكبّرها مع ساري.", "Start with a step.<br>Grow with Sary.")}</h2><p>${t("اختر مساحة تناسب نشاطك من الباقات الحالية.", "Choose the fit for your business from current plans.")}</p>${button("/pricing", t("شاهد الباقات والأسعار", "See plans and pricing"))}</section>${faqSection(catalog["/product/ai-agent"])}${cta()}`;
  const marketing = () =>
